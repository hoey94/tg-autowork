...
# Chat to inspect
# CHAT_LINK = "https://t.me/+pjkIHKL3FYw4OWI1"
...
def tlg_get_all_members(client, chat):
    ...
    while True:
        participants_i = client(GetParticipantsRequest(channel=chat_entity, \
                                                       filter=ChannelParticipantsSearch(''), offset=i,
                                                       limit=num_members, hash=0))
...
def tlg_get_messages(client, chat, num_msg, offset_id=None):
    '''Get all members information from a group/channel/chat'''
    # Set the result list
    messages = []
    # Get the corresponding chat entity
    ...
    if offset_id is None:
        ...
        msgs = list(reversed(client.get_messages(chat_entity, reverse=True, limit=num_msg, offset_id=offset_id)))
...
# Get messages data from a chat
# def tlg_get_messages_by_min_date(client, chat, min_date):
#     '''Get all members information from a group/channel/chat'''
#     # Set the result list
#     messages = []
#     # Get the corresponding chat entity
...
#     # Get and save messages data in a single list
#     print(min_date)
#     msgs = client.iter_messages(chat_entity, reverse=True, offset_date=min_date,search='发')
#     # Build our messages data structures and add them to the list
...
def main(CHAT_LINK):
    ...
    if client is not None:
        ...
        messages = tlg_get_messages(client, CHAT_LINK, 1)
